...
const ExportModal = ({ onClose }) => {
  ...
  // Get createdAt and today's date
  const user = JSON.parse(localStorage.getItem('user'));
  const formatDate = (dateStr) => new Date(dateStr).toISOString().split('T')[0];

  const minDate = formatDate(user?.createdAt || '2024-01-01');
  const maxDate = formatDate(new Date());

  const handleExport = async (resource, formatType) => {
    if (!dateRange.start || !dateRange.end) {
      toast.error('Please select both start and end dates before exporting');
      return;
    }
    ...
    try {
      ...
      const response = await api.get(`/export/${resource}/${formatType}?${params}`, {
        responseType: 'blob',
      });
  ...
  return (
    <Modal onClose={onClose}>
      <div className="bg-white rounded-lg shadow-xl max-w-3xl w-full p-6">
        ...
        {/* Preview Table (optional if needed later) */}
        {previewData && previewResource && (
          <div className="mt-8 overflow-auto max-h-96 border border-gray-200 rounded p-4 bg-gray-50">
            <h4 className="text-lg font-semibold mb-4">
              Preview: {previewResource.charAt(0).toUpperCase() + previewResource.slice(1)} Data
                ...
                {previewData.map((row, i) => (
                  <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-100'}>
                    {Object.entries(row).map(([key, val], j) => (
                      <td key={j} className="border border-gray-300 px-3 py-1 text-sm">
                        {key === 'createdAt' && val
